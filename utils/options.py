...
def parse_options(root_path, is_train=True):
    ...
    for phase, dataset in opt['datasets'].items():
        ...
        if dataset.get('dataroot_cf') is not None:
            dataset['dataroot_cf'] = osp.expanduser(dataset['dataroot_cf'])
        if dataset.get('dataroot_hf') is not None:
            dataset['dataroot_hf'] = osp.expanduser(dataset['dataroot_hf'])
    ...
    if is_train:
        ...
        if results_root is None:
            results_root = osp.join(root_path, 'results_tensor2img_fast')
